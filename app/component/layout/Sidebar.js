        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="space-y-2">
            {conversationHistory.map((item, idx) => (
              <button 
                key={idx} 
                className="w-full p-4 rounded-xl bg-gray-800/50 hover:bg-gray-800 transition-colors text-left border border-gray-700/50"
                onClick={() => onSelectConversation(item)}
              >
                <div className="font-medium text-sm mb-2 text-gray-200">{item.topic}</div>
