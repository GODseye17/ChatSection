...
export default function Sidebar({ 
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-80 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="h-full bg-gray-900 border-r border-gray-800 flex flex-col relative overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-800 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-purple-400 bg-clip-text text-transparent">Vivum</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 relative z-10">
          <div className="space-y-2">
            {conversationHistory.map((item, idx) => (
              <div 
                key={idx} 
                className="p-4 rounded-xl hover:bg-gray-800/50 cursor-pointer transition-colors group relative overflow-hidden backdrop-blur-sm"
        ...
        <div className="p-4 border-t border-gray-800 relative z-10">
          <button className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-800/50 backdrop-blur-sm transition-colors">
